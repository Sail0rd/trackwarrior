...
class Record {
  ...
  constructor(data) {
    ...
    if (this.data.tags) {
      const clearTimeTags = config.getArray(
        "clear_time_tags",
        settings.clearTimeTags,
      );
      const updateTimeTags = config.getArray(
        "update_time_tags",
        settings.clearTimeTags,
      );
  ...
  setDuration(value) {
    ...
    const currencyFormat = config.getArray(
      "currency_format",
    ...
    if (project.length > 0) {
      const ratePerHourProject = config.getArray(
        "rate_per_hour_project",
        settings.ratePerHourProject,
      );
      const rates = {};
      for (let item of ratePerHourProject) {
        const [projectName, projectRate] = item.split(":");
        rates[projectName.trim()] = !isNaN(Number(projectRate.trim()))
          ? Number(projectRate.trim())
          : 0;
      }
    ...
    const decimals = config.getInt(
      "rate_per_hour_decimals",
      settings.ratePerHourDecimals,
    );
    ...
    const ratelFormatted =
      ratePerHour == 0 ? "" : `${rateFormatter.format(ratePerHour)}`;
    this.set("trackwarrior_rate", ratelFormatted);

    const spaces = config.getInt(
      "rate_format_with_spaces",
      settings.rateFormatWithSpaces,
    );
    const totalFormatted =
      total == 0 ? "" : formatWithSpaces(amountFormatter.format(total), spaces);
  ...
  getTags(escaped = false) {
    ...
    let tags = [this.getId(), data.description];
